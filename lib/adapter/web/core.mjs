import { DocumentReference, Timestamp, addDoc, arrayRemove, arrayUnion, collection, deleteDoc, deleteField, doc, documentId, endAt, endBefore, getCountFromServer, getDoc, getDocs, increment, limit, onSnapshot, orderBy, query, serverTimestamp, setDoc, startAfter, startAt, updateDoc, where, and, or, getAggregateFromServer, sum, average } from "firebase/firestore";
...
export class Collection {
  constructor(db, name, path) {
    ...
    this.update = (id, data, options) => {
      ...
      return updateDoc(this.firebaseDoc(id), unwrapData(this.firestore, update)).then(() => this.ref(id));
    };
    this.update.build = (id, options) => {
      ...
      return {
        ...
        run: () => updateDoc(this.firebaseDoc(id), unwrapData(this.firestore, updateFields(fields))).then(() => this.ref(id))
      };
    };
  ...
  id(id) {
    if (id) return id;else return Promise.resolve(doc(this.firebaseCollection()).id);
  }
  ref(id) {
  ...
  add(data, options) {
    assertEnvironment(options?.as);
    return addDoc(this.firebaseCollection(), writeData(this.firestore, data)).then(firebaseRef => this.ref(firebaseRef.id));
  }
  set(id, data, options) {
    assertEnvironment(options?.as);
    return setDoc(this.firebaseDoc(id), writeData(this.firestore, data)).then(() => this.ref(id));
  }
  ...
  get(id, options) {
    ...
    return new SubscriptionPromise({
      ...
      subscribe: (onResult, onError) => onSnapshot(doc, firebaseSnap => {
        const data = firebaseSnap.data();
...
export function all(adapter) {
  ...
  return new SubscriptionPromise({
    ...
    subscribe: (onResult, onError) => onSnapshot(firebaseCollection, firebaseSnap => {
...
function subShortcut(firestore, schema) {
  return Object.entries(schema).reduce((shortcutsSchema, [path, schemaCollection]) => {
    shortcutsSchema[path] = {
      id(id) {
        if (id) return id;else return Promise.resolve(doc(collection(firestore(), "any")).id);
      }
    };
...
export function _query(firestore, adapter, queries) {
      ...
      case "order":
        {
          ...
          firebaseRestQueries.push(orderBy(field[0] === "__id__" ? documentId() : field.join("."), method));
      ...
      case "where":
        {
          ...
          firebaseWhereQueries.push(where(wherePath(field), filter, unwrapData(firestore, value)));
          break;
        }
      case "limit":
        {
      ...
      case "or":
        {
          ...
          firebaseWhereQueries.push(or(...query.queries.map(q => where(wherePath(q.field), q.filter, unwrapData(firestore, q.value)))));
          break;
        }
    }
  }, []);
  ...
  if (cursors.length && cursors.every(cursor => cursor.value !== undefined)) groupedCursors.forEach(([method, values]) => {
    firebaseCursors.push((method === "startAt" ? startAt : method === "startAfter" ? startAfter : method === "endAt" ? endAt : endBefore)(...values));
  });
  const firebaseQuery = () => query(adapter.collection(), and(...firebaseWhereQueries), ...firebaseRestQueries, ...firebaseCursors);
  const sp = new SubscriptionPromise({
    ...
    subscribe: (onResult, onError) => {
      ...
      return onSnapshot(q, firebaseSnap => {
