...
import { Collection, Doc, Ref, UpdateField, assertEnvironment, pathRegExp, unwrapData, updateFields, updateHelpers, wrapData, writeHelpers } from "./core.mjs";
...
export const transaction = (db, options) => {
  ...
  return {
    read: readCallback => {
      return {
        write: writeCallback => runTransaction(db[firestoreSymbol](), async firebaseTransaction => {
...
class ReadCollection {
  ...
  async get(id) {
    ...
    return new ReadDoc(this, id, wrapData(this.db, snapshot.data(), (db, path) => pathToWriteRef(db, this.transaction, path)));
  }
}
class ReadRef {
...
function writeDocsToDocs(db, value) {
  if (value instanceof WriteDoc) {
    return new Doc(new Collection(db, value.ref.collection.name, value.ref.collection.path), value.ref.id, value.data);
  } else if (value instanceof WriteRef) {
    return new Ref(new Collection(db, value.ref.collection.name, value.ref.collection.path), value.ref.id);
