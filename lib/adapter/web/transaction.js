...
const transaction = (db, options) => {
  ...
  return {
    read: readCallback => {
      return {
        write: writeCallback => (0, _firestore.runTransaction)(db[_firebase.firestoreSymbol](), async firebaseTransaction => {
...
function firebaseDoc(db, path, id) {
  return (0, _firestore.doc)(db, `${path}/${id}`);
}
...
class WriteCollection {
  ...
  update(id, data) {
    ...
    const doc = firebaseDoc(this.firestore(), this.path, id);
    this.transaction.update(doc, (0, _core.unwrapData)(this.firebaseDB, update));
  }
  remove(id) {
    const doc = firebaseDoc(this.firestore(), this.path, id);
...
function writeDocsToDocs(db, value) {
  if (value instanceof WriteDoc) {
    ...
    return new _core.Ref(new _core.Collection(db, value.ref.collection.name, value.ref.collection.path), value.ref.id);
