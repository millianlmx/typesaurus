import { collectionGroup, getCountFromServer, getAggregateFromServer, sum, average } from "firebase/firestore";
...
class Group {
  constructor(db, name) {
    ...
    this.name = name;
    this.query = queries => _query(this.firestore, this.adapter(), [].concat(queries(queryHelpers())));
    this.query.build = () => {
      const queries = [];
      return {
  ...
  async count() {
    const snap = await getCountFromServer(collectionGroup(this.firestore(), this.name));
    return snap.data().count;
  }
  async sum(field) {
    const snap = await getAggregateFromServer(collectionGroup(this.firestore(), this.name), {
      result: sum(field)
    });
  ...
  adapter() {
    return {
      ...
      doc: snapshot => pathToDoc(this.db, snapshot.ref.path, wrapData(this.db, snapshot.data())),
      request: () => ({
        path: this.name,
        group: true
      })
