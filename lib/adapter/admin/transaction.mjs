...
export const transaction = (db, options) => {
  ...
  return {
    read: readCallback => {
      return {
        write: writeCallback => db[firestoreSymbol]().runTransaction(async firebaseTransaction => {
...
function writeDocsToDocs(db, value) {
  if (value instanceof WriteDoc) {
    ...
    return new Ref(new Collection(value.ref.collection.name, value.ref.collection.path), value.ref.id);
