...
exports.dbSymbol = void 0;
exports.getRefPath = getRefPath;
exports.pathRegExp = exports.nativeSymbol = void 0;
...
class Collection {
  constructor(db, name, path) {
    ...
    this.update = (id, data, options) => {
      ...
      return (Object.keys(update).length ? this.firebaseDoc(id).update(unwrapData(this.firestore, update)) : Promise.resolve()).then(() => this.ref(id));
    };
    this.update.build = (id, options) => {
      ...
      return {
        ...
        run: async () => {
          if (fields.length) await this.firebaseDoc(id).update(unwrapData(this.firestore, updateFields(fields)));
          return this.ref(id);
        }
      };
    };
    this.query = (queries, options) => {
      ...
      return query(this.firestore, this.adapter(), [].concat(queriesResult).filter(q => !!q));
    };
  ...
  id(id) {
    if (id) return id;else return Promise.resolve(this.firebaseCollection().doc().id);
  }
  ref(id) {
  ...
  doc(id, value, options) {
    if (!value && "id" in id && "data" in id && typeof id.data === "function") {
      if (id.exists) return this.doc(id.id, wrapData(this.db, id.data()));else return null;
    } else {
      assertEnvironment(options?.as);
  ...
  add(data, options) {
    assertEnvironment(options?.as);
    return this.firebaseCollection().add(writeData(this.firestore, data)).then(firebaseRef => this.ref(firebaseRef.id));
  }
  set(id, data, options) {
    assertEnvironment(options?.as);
    return this.firebaseDoc(id).set(writeData(this.firestore, data)).then(() => this.ref(id));
  }
  upset(id, data, options) {
    assertEnvironment(options?.as);
    return this.firebaseDoc(id).set(writeData(this.firestore, data), {
      merge: true
    }).then(() => this.ref(id));
  }
  async remove(id) {
  ...
  get(id, options) {
    ...
    return new _index.SubscriptionPromise({
      ...
      subscribe: (onResult, onError) => doc.onSnapshot(firebaseSnap => {
        const data = firebaseSnap.data();
  ...
  many(ids, options) {
    ...
    return new _index.SubscriptionPromise({
      ...
      get: async () => {
        ...
        const firebaseSnap = await this.firestore().getAll(...ids.map(id => this.firebaseDoc(id)));
...
function all(adapter) {
  ...
  return new _index.SubscriptionPromise({
    ...
    subscribe: (onResult, onError) => firebaseCollection.onSnapshot(firebaseSnap => {
...
function subShortcut(firestore, schema) {
  return Object.entries(schema).reduce((shortcutsSchema, [path, schemaCollection]) => {
    shortcutsSchema[path] = {
      id(id) {
        if (id) return id;else return Promise.resolve(firestore().collection("any").doc().id);
      }
    };
...
function query(firestore, adapter, queries) {
      ...
      case "order":
        {
          ...
          firestoreQuery = firestoreQuery.orderBy(field[0] === "__id__" ? _firestore.FieldPath.documentId() : field.join("."), method);
      ...
      case "where":
        {
          ...
          firestoreQuery = firestoreQuery.where(wherePath(field), filter, unwrapData(firestore, value));
          break;
        }
      case "limit":
        {
      ...
      case "or":
        {
          ...
          firestoreQuery = firestoreQuery.where(_firestore.Filter.or(...query.queries.map(q => _firestore.Filter.where(wherePath(q.field), q.filter, unwrapData(firestore, q.value)))));
          break;
        }
    }
  });
  ...
  const sp = new _index.SubscriptionPromise({
    ...
    subscribe: (onResult, onError) => firestoreQuery.onSnapshot(firebaseSnap => {
  ...
  Object.assign(sp, {
    ...
    sum: async field => {
      const snap = await firestoreQuery.aggregate({
        result: _firestore.AggregateField.sum(field)
      }).get();
      return snap.data().result;
    },
...
function wherePath(field) {
  return field[0] === "__id__" ? _firestore.FieldPath.documentId() : field.join(".");
}
  ...
  if (data && typeof data === "object") {
    if (data.type === "ref") {
      ...
      switch (fieldValue.kind) {
        case "remove":
          return _firestore.FieldValue.delete();
        case "increment":
          return _firestore.FieldValue.increment(fieldValue.number);
        case "arrayUnion":
          return _firestore.FieldValue.arrayUnion(...unwrapData(firestore, fieldValue.values));
        case "arrayRemove":
          return _firestore.FieldValue.arrayRemove(...unwrapData(firestore, fieldValue.values));
        case "serverDate":
          return _firestore.FieldValue.serverTimestamp();
      }
...
function assertEnvironment(environment) {
  if (environment && environment !== "server") throw new Error(`Expected ${environment} environment`);
}
