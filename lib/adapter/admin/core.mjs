import { DocumentReference, FieldPath, FieldValue, Timestamp, Filter, AggregateField } from "firebase-admin/firestore";
import { SubscriptionPromise } from "../../sp/index.mjs";
...
export class Collection {
  ...
  async sum(field) {
    const snap = await this.adapter().collection().aggregate({
      result: AggregateField.sum(field)
    }).get();
    return snap.data().result;
  }
  async average(field) {
    const snap = await this.adapter().collection().aggregate({
      result: AggregateField.average(field)
    }).get();
    return snap.data().result;
  }
  adapter() {
      ...
      case "order":
        {
          ...
          firestoreQuery = firestoreQuery.orderBy(field[0] === "__id__" ? FieldPath.documentId() : field.join("."), method);
      ...
      case "or":
        {
          ...
          firestoreQuery = firestoreQuery.where(Filter.or(...query.queries.map(q => Filter.where(wherePath(q.field), q.filter, unwrapData(firestore, q.value)))));
          break;
        }
    }
  });
  ...
  if (data && typeof data === "object") {
    if (data.type === "ref") {
      ...
      switch (fieldValue.kind) {
        ...
        case "arrayUnion":
          return FieldValue.arrayUnion(...unwrapData(firestore, fieldValue.values));
        case "arrayRemove":
          return FieldValue.arrayRemove(...unwrapData(firestore, fieldValue.values));
        case "serverDate":
